...
install_system_packages() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        local termux_pkgs=(clang rust make pkg-config libffi openssl ca-certificates curl python-cryptography python-pillow)
...
setup_venv() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Termux ships ABI-matched native modules such as cryptography. Make
        # them visible inside the venv so pip does not rebuild Rust extensions
        # from Android-incompatible PyPI sdists when no wheel exists.
