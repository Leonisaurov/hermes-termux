...
def test_initializer_runs_on_python_worker_api():
    """The pool keeps initializer support across Python stdlib layouts."""
    ...
    try:
        assert pool.submit(lambda: getattr(marker, "ready", False)).result(timeout=10)
    finally:
        pool.shutdown(wait=True)
