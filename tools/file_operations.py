...
class ShellFileOperations(FileOperations):
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 2000) -> ReadResult:
        ...
        if policy_error:
            return ReadResult(error=policy_error)
        ...
        if policy_error:
            return ReadResult(error=policy_error)
        # Strict Termux: resolve to absolute + follow symlinks before the
        # actual read so a symlinked escape is also blocked.
        try:
            _resolved = os.path.realpath(path)
            _policy = path_policy_error(_resolved, operation="read")
            if _policy:
                return ReadResult(error=_policy)
        except Exception:
            pass
    ...
    def read_file_raw(self, path: str) -> ReadResult:
        ...
        if policy_error:
            return ReadResult(error=policy_error)
        # Strict Termux: follow symlinks before the actual read.
        try:
            _resolved = os.path.realpath(path)
            _policy = path_policy_error(_resolved, operation="read")
            if _policy:
    ...
    def _python_delete(self, path: str, recursive: bool) -> WriteResult:
        path = self._expand_path(path)
        # Strict Termux: block deletes outside the Termux home.
        try:
            from tools.termux_path_policy import path_policy_error
            _err = path_policy_error(path, operation="delete")
            if _err:
                return WriteResult(error=_err)
        except Exception:
            pass
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        for p in (src, dst):
            # Strict Termux: block moves outside the Termux home.
            try:
                from tools.termux_path_policy import path_policy_error
                _err = path_policy_error(p, operation="move")
                if _err:
                    return WriteResult(error=_err)
            except Exception:
                pass
    ...
    def write_file(self, path: str, content: str,
        ...
        # Strict Termux: block writes outside the Termux home.
        try:
            from tools.termux_path_policy import path_policy_error
            _err = path_policy_error(path, operation="write")
            if _err:
                return WriteResult(error=_err)
        except Exception:
            pass
