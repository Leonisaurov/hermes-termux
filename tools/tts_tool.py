...
from hermes_constants import display_hermes_home, is_termux
...
TERMUX_TTS_COMMAND = "termux-tts-speak"
...
PROVIDER_MAX_TEXT_LENGTH: Dict[str, int] = {
    ...
    "termux": 5000,       # Android Termux:API accepts text through stdin
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    """Resolve the TTS provider, preferring Android's local Termux bridge."""
    configured = (tts_config.get("provider") or "").lower().strip()
    # Treat the historical Edge default as implicit on Termux. Explicit
    # choices such as OpenAI remain available as opt-in alternatives.
    if is_termux() and configured in {"", "edge"}:
        return "termux"
    return configured or DEFAULT_PROVIDER


def _termux_tts_available() -> bool:
    """Return whether Android's Termux:API TTS command is installed."""
    return is_termux() and shutil.which(TERMUX_TTS_COMMAND) is not None


def _speak_with_termux(
    text: str,
    tts_config: Dict[str, Any],
    *,
    speed: Optional[float] = None,
) -> None:
    """Speak text through Android's TTS engine via Termux:API.

    The command accepts text on stdin, avoiding shell quoting and argument
    length issues.  This backend intentionally produces no audio file: the
    Android system speaks directly through the selected audio stream.
    """
    if not _termux_tts_available():
        raise FileNotFoundError(
            "termux-tts-speak no está disponible; instala Termux:API y el paquete "
            "termux-api"
        )

    section = tts_config.get("termux") or {}
    if not isinstance(section, dict):
        section = {}
    command = [TERMUX_TTS_COMMAND]
    options = {
        "-e": section.get("engine"),
        "-l": section.get("language"),
        "-n": section.get("region"),
        "-v": section.get("variant"),
        "-p": section.get("pitch"),
        "-s": section.get("stream"),
    }
    for flag, value in options.items():
        if value is not None and str(value).strip():
            command.extend((flag, str(value)))
    rate = speed if speed is not None else section.get("rate", tts_config.get("speed"))
    if rate is not None:
        command.extend(("-r", str(max(0.1, min(4.0, float(rate))))))
    ...
    if result.returncode != 0:
        detail = (result.stderr or result.stdout or "sin detalles").strip()
        raise RuntimeError(f"termux-tts-speak terminó con código {result.returncode}: {detail}")
...
def text_to_speech_tool(
    ...
    if provider:
        ...
        provider = _get_provider(tts_config)
    # Android's Termux:API TTS speaks directly through the device and does
    # not create an audio artifact for Hermes to upload or play afterward.
    # Handle it before the normal file-oriented provider pipeline.
    if provider == "termux":
        max_len = _resolve_max_text_length(provider, tts_config)
        chunks = _split_text_for_tts(text, max_len)
        try:
            for chunk in chunks:
                _speak_with_termux(chunk, tts_config, speed=speed)
            return json.dumps({
                "success": True,
                "provider": "termux",
                "spoken": True,
                "chunk_count": len(chunks),
                "message": "Texto reproducido mediante Android TTS (Termux:API).",
            }, ensure_ascii=False)
        except (FileNotFoundError, RuntimeError, subprocess.TimeoutExpired, ValueError) as exc:
            return tool_error(f"Termux TTS failed: {exc}", success=False)
