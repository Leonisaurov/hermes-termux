...
def _termux_api_app_installed() -> bool:
    ...
    if inconclusive and (
        shutil.which("termux-microphone-record") is not None
        or shutil.which("termux-speech-to-text") is not None
    ):
        logger.debug(
...
def _termux_voice_capture_available() -> bool:
    return (
        (_termux_microphone_command() is not None
         or _termux_speech_to_text_command() is not None)
        and _termux_api_app_installed()
    )


def _termux_speech_to_text_command() -> Optional[str]:
    if not _is_termux_environment():
        return None
    return shutil.which("termux-speech-to-text")


def _termux_speech_recorder_enabled() -> bool:
    """Use the Android recognizer as the live recorder when it is default STT."""
    command = _termux_speech_to_text_command()
    if not command:
        return False
    try:
        from tools.transcription_tools import _get_provider, _load_stt_config

        return _get_provider(_load_stt_config()) == "termux"
    except Exception:
        return False
...
# ============================================================================
# Termux speech recognizer recorder
# ============================================================================
class TermuxSpeechToTextRecorder:
    """Live recorder backed directly by ``termux-speech-to-text``."""
    ...
    def start(self, on_silence_stop=None) -> None:
        del on_silence_stop
        command = _termux_speech_to_text_command()
        if not command or not _termux_api_app_installed():
            raise RuntimeError(
                "Termux speech recognition requires the Termux:API app and "
                "the termux-api package. Install with: pkg install termux-api"
            )
        with self._lock:
            if self._recording:
                return
            try:
                ...
                raise RuntimeError(f"Termux speech recognition failed: {exc}") from exc
            self._start_time = time.monotonic()
            self._recording = True
            self._current_rms = 0
        logger.info("Termux speech-to-text recording started")
    ...
    def stop(self) -> Optional[str]:
        ...
        if process.returncode not in (0, None):
            logger.warning(
                "termux-speech-to-text exited with code %s: %s",
                process.returncode,
                (stderr or stdout or "sin detalles").strip(),
            )
            return None
        transcript = next(
            (line.strip() for line in reversed((stdout or "").splitlines()) if line.strip()),
            "",
        )
        if not transcript:
            return None
        os.makedirs(_TEMP_DIR, exist_ok=True)
        path = os.path.join(_TEMP_DIR, f"termux_stt_{time.time_ns()}.termux-stt.txt")
        Path(path).write_text(transcript, encoding="utf-8")
        logger.info("Termux speech-to-text recording stopped")
        return path
...
def create_audio_recorder() -> AudioRecorder | TermuxAudioRecorder | TermuxSpeechToTextRecorder:
    """Return the best recorder backend for the current environment."""
    if _termux_speech_recorder_enabled():
        return TermuxSpeechToTextRecorder()
...
def transcribe_recording(wav_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    if str(wav_path).endswith(".termux-stt.txt"):
        try:
            transcript = Path(wav_path).read_text(encoding="utf-8").strip()
        except OSError as exc:
            return {
                "success": False,
                "transcript": "",
                "provider": "termux",
                "error": f"Could not read Termux transcription: {exc}",
            }
        return {
            "success": True,
...
def check_voice_requirements() -> Dict[str, Any]:
    ...
    from tools.transcription_tools import (
        ...
        _termux_speech_to_text_available,
        is_stt_enabled,
    )
    ...
    if stt_provider == "termux":
        native_stt_available = _termux_speech_to_text_available()
    ...
    termux_stt_capture = _termux_speech_to_text_available()
    has_audio = _audio_available() or termux_capture or termux_stt_capture
    ...
    if stt_provider == "termux" and termux_stt_capture:
        details_parts.append("Audio capture: OK (Termux:API speech recognition)")
    elif termux_capture:
    ...
    elif stt_provider == "termux":
        details_parts.append("STT provider: OK (Termux:API speech recognition)")
