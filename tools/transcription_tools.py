...
TERMUX_STT_COMMAND = "termux-speech-to-text"
...
def _termux_speech_to_text_available() -> bool:
    """Return whether Termux:API's interactive STT command is installed."""
    return is_termux() and shutil.which(TERMUX_STT_COMMAND) is not None


def _transcribe_termux_speech(stt_config: Dict[str, Any]) -> Dict[str, Any]:
    """Capture one utterance with Android's Termux:API speech recognizer.

    ``termux-speech-to-text`` owns microphone capture and returns the final
    recognition on stdout. It cannot transcribe an existing audio file; the
    file argument is retained by the public API for compatibility with other
    STT providers and is intentionally not uploaded or read here.
    """
    if not _termux_speech_to_text_available():
        return {
            "success": False,
            "transcript": "",
            "provider": "termux",
            "error": (
                "termux-speech-to-text no está disponible; instala Termux:API "
                "y el paquete termux-api"
            ),
        }

    section = stt_config.get("termux") or {}
    ...
    command = [TERMUX_STT_COMMAND]
    if section.get("progress"):
        command.append("-p")
    try:
        ...
        return {
            ...
            "transcript": "",
            "provider": "termux",
            "error": f"Termux speech recognition timed out after {timeout:.0f}s",
        }
        ...
        return {
            ...
            "transcript": "",
            "provider": "termux",
            "error": f"Termux speech recognition failed: {exc}",
        }

    output = (result.stdout or "").strip()
    # The default Termux:API wrapper already emits only the final line. With
    # progress enabled, partial matches may precede it, so use the last
    # non-empty line in either mode.
    transcript = next((line.strip() for line in reversed(output.splitlines()) if line.strip()), "")
    if result.returncode != 0:
        detail = (result.stderr or output or "sin detalles").strip()
        return {
            "success": False,
            "transcript": "",
            "provider": "termux",
            "error": f"termux-speech-to-text terminó con código {result.returncode}: {detail}",
        }
    return {
        "success": True,
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    ...
    if key in {"termux", "local", "local_command"}:
        return True
    return False
...
def _get_provider(stt_config: dict) -> str:
    ...
    # Termux:API is the on-device default for Termux. The historical local
    # Whisper value is treated as implicit here; cloud providers remain
    # available when explicitly selected by the user.
    if is_termux() and provider_key in {"", "local"}:
        if _termux_speech_to_text_available():
            return "termux"
        logger.warning(
            "Termux STT command unavailable; continuing with the configured local provider"
        )
...
CLOUD_STT_PROVIDERS = frozenset(
    BUILTIN_STT_PROVIDERS - {"termux", "local", "local_command"}
)
...
def _transcribe_prepared_audio(
    ...
    # Strict Termux: refuse any path outside the Termux home.
    try:
        from tools.termux_path_policy import path_policy_error
        _err = path_policy_error(file_path, operation="read")
        if _err:
            return {"success": False, "transcript": "", "error": _err}
    except Exception:
        pass
...
def _dispatch_stt_provider(
    ...
    if provider == "termux":
        return _transcribe_termux_speech(stt_config)
