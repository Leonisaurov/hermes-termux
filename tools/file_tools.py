...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    try:
        ...
        for candidate in (filepath, resolved, normalized):
            policy_error = path_policy_error(candidate, operation="write")
            if policy_error:
                return policy_error
    except Exception:
        pass
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Termux path policy (strict) ─────────────────────────────────
        # In Termux the agent may only touch files inside the Termux home
        # directory tree.  This is an explicit allow-list — every path that
        # resolves outside /data/data/com.termux/files/home is refused,
        # regardless of the operation.  Applied AFTER resolution so symlinks
        # that point out of the home are also caught.
        try:
            from tools.termux_path_policy import path_policy_error
            _policy_err = path_policy_error(str(_resolved), operation="read")
            if _policy_err:
                return tool_error(_policy_err)
        except Exception:
            pass
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        from tools.termux_path_policy import path_policy_error
        policy_error = path_policy_error(path, operation="read")
        if policy_error:
            return tool_error(policy_error)
        try:
            resolved_for_policy = str(_resolve_path_for_task(path, task_id))
        except (OSError, ValueError, RuntimeError):
            resolved_for_policy = path
        policy_error = path_policy_error(resolved_for_policy, operation="read")
        if policy_error:
            return tool_error(policy_error)
