...
def _get_env_config() -> Dict[str, Any]:
    ...
    # Strict Termux path policy: if TERMINAL_CWD points outside the Termux
    # home directory, refuse to use it and fall back to the default (process
    # cwd / home).  This prevents a misconfigured or poisoned env var from
    # dropping the agent into a system directory it should never touch.
    if env_type == "local":
        try:
            from tools.termux_path_policy import path_policy_error
            _cwd_err = path_policy_error(cwd, operation="access")
            if _cwd_err:
                logger.warning("TERMINAL_CWD %r blocked by Termux policy; falling back to %r", cwd, default_cwd)
                cwd = default_cwd
        except Exception:
            pass
    host_cwd = None
...
def terminal_tool(
    ...
    try:
        ...
        # Termux has no OS-level sandbox in the local backend. Keep this
        # fence outside the approval/force path so --yolo or a confirmation
        # cannot widen it accidentally.
        if env_type == "local":
            from tools.termux_path_policy import command_policy_error
