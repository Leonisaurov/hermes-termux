...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    # Termux packages and runtime files are outside the agent's writable area.
    try:
        from tools.termux_path_policy import prefix_path
        if prefix_path(resolved):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "termux_prefix":
        return f"{verb} denied: the Termux installation prefix ($PREFIX) is protected."
